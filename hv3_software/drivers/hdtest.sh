...
run_bsm_Write ()
{
	...
	((loop_index=0))
	for ((ms=$first_lba; ms<$((first_lba+addr_inc*page_count)); ms=$ms+$addr_inc))
	do
...
echo 700 > /sys/kernel/debug/$DEBUG_FS/bsm_wrt_delay_before_qc		# 700 us delay for eMMC operation
