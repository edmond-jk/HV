...
//MK0627-end
//MK20170717-begin
//static unsigned long timestamp2[10];
unsigned long timestamp2[10];
//MK20170717-end
...
//MK0324-end
EXPORT_SYMBOL(bsm_read_command_emmc);
...
//MK20170717	pr_debug("[%s]: mmio_retry_count = %d  retry_count = %d\n", __func__, mmio_retry_count, retry_count);
//MK0215-end
...
//MK20170717		pr_debug("[%s]: bsm_rd_cmd_checksum_verification disabled\n", __func__);
	}
//MK0223-end
...
//XXX 
//	pr_debug("[%s]: =ET= (retry_count=%d)  LBA=0x%.8X TOTAL:%.8ld ns CMD:%.8ld ns QC:%.8ld ns FAKE:%.8ld ns CS:%.8ld ns\n",
...
//	pr_debug("[%s]: =ET2= LBA=0x%.8X TOTAL:%.8ldns [0~1]:%.8ldns [1~2]:%.8ldns [2~3]:%.8ldns [3~4]:%.8ldns [4~5]:%.8ldns\n",
//			__func__, pBsm->lba[0],
//			timestamp2[5]-timestamp2[0], timestamp2[1]-timestamp2[0],
//			timestamp2[2]-timestamp2[1], timestamp2[3]-timestamp2[2],
//			timestamp2[4]-timestamp2[3], timestamp2[5]-timestamp2[4]);
//MK20170717-end
...
//MK20170717		pr_debug("[%s]: bsm_wrt_cmd_checksum_verification disabled\n", __func__);
	}
//MK0223-end
...
//MK0627-begin
	timestamp[4] = hv_nstimeofday();
//XXX
//	pr_debug("[%s]: =ET= (retry_count=%d)  LBA=0x%.8X TOTAL:%.8ld ns CMD:%.8ld ns FAKE:%.8ld ns QC:%.8ld ns CS:%.8ld ns\n",
//			__func__, retry_count, pBsm->lba[0], timestamp[4]-timestamp[0],
//			timestamp[1]-timestamp[0], timestamp[2]-timestamp[1],
//			timestamp[3]-timestamp[2], timestamp[4]-timestamp[3]);
//MK0627-end
