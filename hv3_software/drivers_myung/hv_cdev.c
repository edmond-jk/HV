...
#include <linux/version.h>			//MK20170725
...
const char* HV_CDEV_CLASS_NAME[] = {"hv_cdev_class_bsm", "hv_cdev_class_mmls" };
const char* HV_CDEV_NAME[] = {"hv_cdev_bsm", "pmem" };

static int hv_cdev_major[HV_CDEV_NUM];
...
typedef struct privatedata {
	...
	void __iomem *iomem;		/* kernel virtual address ptr of iomem   */
	unsigned long pfn;		/* page frame number of physical mem */
	unsigned long nsectors;		/* mmls num of sectors */
	int use_static_buff;		/* use static buffer is no HW or RAMDISK */
	int hv_cdev_type;		/* BSM or MMLS */
} hv_cdev_private;

hv_cdev_private devices[HV_CDEV_NUM][HV_CDEV_N_MINORS];
	...
	char __user *ubuff, size_t count, loff_t *f_pos)
{
	...
	if (priv->use_static_buff) {
		...
		if (!get_ramdisk()) {
			kbuff = priv->buff;
			if (priv->hv_cdev_type == HV_CDEV_MMLS)
				/* kbuff has to be on HVDIMM DRAM */
				mmls_read_command(1, count/512, (*f_pos)/512, (long)kbuff, 0, NULL);
			else if (priv->hv_cdev_type == HV_CDEV_BSM)
//MK0207				bsm_read_command(1, count/512, (*f_pos)/512, kbuff, 0, NULL);
//MK0207-begin
				bsm_read_command(1, count/512, (*f_pos)/512, kbuff, 0, NULL, 0);
//MK0207-end
		}
		else {
			kbuff = priv->iomem + *f_pos;
		}
		if (copy_to_user(ubuff, kbuff, count)) {
			n = -EFAULT;
			PERR("Error: Copy_to_user failed\n");
			goto out;
		}
	...
	const char __user *ubuff, size_t count, loff_t *f_pos)
{
	...
	if (priv->use_static_buff) {
//MK20170725		if (__copy_from_user_nocache(priv->buff + *f_pos, ubuff, count)) {
		if (__copy_user_nocache(priv->buff + *f_pos, ubuff, count, 1)) {		//MK20170725
			n = -EFAULT;
		...
		if (!get_ramdisk())
            ...
            //PINFO("priv->iomem + *f_pos=%p\n", priv->iomem + *f_pos);
            /* priv->iomem is ioremap(physical ram addr) done in hv_mmio.c */
			kbuff = priv->iomem + *f_pos;
        }

//MK20170725		if (__copy_from_user_nocache(kbuff, ubuff, count)) {
		if (__copy_user_nocache(kbuff, ubuff, count, 1)) {						//MK20170725
			n = -EFAULT;
			PERR("Error: Copy_from_user failed\n");
			goto out;
		}
		...
		if (!get_ramdisk()) {
			if (priv->hv_cdev_type == HV_CDEV_MMLS)
				/* kbuff has to be on HVDIMM DRAM */
                /* second param is block_size */
                /* third param is LBA#. Divided by 512, because passed value is in bytes */
				mmls_write_command(1, count/512, (*f_pos)/512, (long)kbuff, 0, NULL);
			else if (priv->hv_cdev_type == HV_CDEV_BSM)
				bsm_write_command(1, count/512, (*f_pos)/512, kbuff, 0, NULL);
		}
	}
		...
		struct file *filp, unsigned int cmd , unsigned long arg)
{
	...
	case HV_MMLS_DUMP_MEM:
	{
		...
		PDEBUG("iomem=%p, offset=%llu, size=%llu\n",
				priv->iomem,
				range.offset,
				range.size);
		...
		for (i = 0 + range.offset; i < range.size; i++)
			pr_info("iomem[%d]=0x%02X\n", i, ioread8(priv->iomem+i));
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4,12,00))		//MK20170725
static int
 ...
 * bsm_init()
 *
 * This function returns bsm size and phys addr from hv cmd
 * driver.
 */
static int bsm_init(void)
{
	PINFO("%s: enter\n", __func__);

	get_bsm_iodata(&bsm_io_data);

	PINFO("%s: found bsm device with size and addr:\n", __func__);
	PINFO("bsm_io_data.b_size=%lu\n", bsm_io_data.b_size);
	PINFO("bsm_io_data.b_iomem=%p\n", bsm_io_data.b_iomem);
	PINFO("nsectors=%lu", (bsm_io_data.b_size)/HV_BLOCK_SIZE);
...
static int mmls_init(void)
{
	...
	PINFO("nsectors=%lu", (mmls_io_data.m_size)/HV_BLOCK_SIZE);

	return 0;
}

int __init hv_cdev_init(int cdev_type)
{
	int i, j;
	int res;
	...
	hv_cdev_major[cdev_type] = MAJOR(hv_cdev_device_num);
	...
	for (i = 0; i < HV_CDEV_N_MINORS; i++) {
		...
		if (cdev_type == HV_CDEV_MMLS) {
			if (mmls_init()) {
				...
				/* size and addresses.				*/
				devices[cdev_type][i].nsectors =
					(mmls_io_data.m_size)/HV_BLOCK_SIZE;

				devices[cdev_type][i].dev_size = mmls_io_data.m_size;
				devices[cdev_type][i].phys_start = mmls_io_data.phys_start;
				devices[cdev_type][i].iomem = mmls_io_data.m_iomem;
				devices[cdev_type][i].use_static_buff = 0;
			}
		}
		else if (cdev_type == HV_CDEV_BSM) {
			if (bsm_init()) {
				PERR("%s: bsm size module parm is 0.\n", __func__);
				PERR("No ramdisk. Use static buff for test\n");
				devices[cdev_type][i].dev_size = HV_CDEV_BUFF_SIZE;

				/* This needs to be in priv data if >1 bsm dev	*/
				devices[cdev_type][i].use_static_buff = 1;
			} else {
				/* Populate private data with bsm device info	*/
				/* Assumed only 1 bsm device.			*/
				/* In the future, if mobsmre bsm devices are used */
				/* with minor #, then bsm_init fn need change	*/
				/* to return more devices with their specific	*/
				/* size and addresses.				*/
				devices[cdev_type][i].nsectors =
					(bsm_io_data.b_size)/HV_BLOCK_SIZE;

				devices[cdev_type][i].dev_size = bsm_io_data.b_size;
				devices[cdev_type][i].phys_start = bsm_io_data.phys_start;
				devices[cdev_type][i].iomem = bsm_io_data.b_iomem;
				devices[cdev_type][i].use_static_buff = 0;
			}
		}

		hv_cdev_device_num = MKDEV(hv_cdev_major[cdev_type], HV_CDEV_FIRST_MINOR+i);

		cdev_init(&devices[cdev_type][i].cdev , &hv_cdev_fops);
		devices[cdev_type][i].cdev.owner = THIS_MODULE;
		devices[cdev_type][i].hv_cdev_type = cdev_type;

		res = cdev_add(&devices[cdev_type][i].cdev, hv_cdev_device_num, 1);
		if (res < 0) {
			PERR("%s: Failed to add char dev\n", __func__);
			return -ENODEV;
		}

		devices[cdev_type][i].nminor = HV_CDEV_FIRST_MINOR+i;

		PINFO("hv_cdev is registered with major#=%d, minor#=%d\n",
				hv_cdev_major[cdev_type],
				HV_CDEV_FIRST_MINOR+i);
		PINFO("hv_cdev owner = %p\n", devices[cdev_type][i].cdev.owner);

		/* Add virtual device class for file access to driver */
		devices[cdev_type][i].hv_cdev_class =
			class_create(THIS_MODULE, HV_CDEV_CLASS_NAME[cdev_type]);

		if (IS_ERR(devices[cdev_type][i].hv_cdev_class)) {
			PERR("%s: Failed to create hv_cdev class\n", __func__);
			res = PTR_ERR(devices[cdev_type][i].hv_cdev_class);
			goto failed_classreg;
		}

		devices[cdev_type][i].hv_cdev_device = device_create(
					devices[cdev_type][i].hv_cdev_class,
					NULL,
					MKDEV(hv_cdev_major[cdev_type], i),
					NULL,
					"%s" "%d",
					HV_CDEV_NAME[cdev_type], i);

		if (IS_ERR(devices[cdev_type][i].hv_cdev_device)) {
			PERR("Failed to create device '%s_%s%d'\n",
					HV_CDEV_CLASS_NAME[cdev_type],
					HV_CDEV_NAME[cdev_type],
					i);
			res = PTR_ERR(devices[cdev_type][i].hv_cdev_device);
			goto failed_devreg;
		}

		mutex_init(&devices[cdev_type][i].cmutex);
	}
...
failed_devreg:
	...
	for (j = 0; j <= i; j++) {
		hv_cdev_device_num = MKDEV(hv_cdev_major[cdev_type], HV_CDEV_FIRST_MINOR+j);
		device_destroy(devices[cdev_type][j].hv_cdev_class, hv_cdev_device_num);
		class_unregister(devices[cdev_type][j].hv_cdev_class);
		class_destroy(devices[cdev_type][j].hv_cdev_class);
	}

failed_classreg:
	unregister_chrdev(hv_cdev_major[cdev_type], HV_CDEV_CLASS_NAME[cdev_type]);

	return res;
}
void __exit hv_cdev_exit(int cdev_type)
{
	int i;
	dev_t hv_cdev_device_num;	/* contains major and minor #. For temp var */
	...
	for (i = 0; i < HV_CDEV_N_MINORS; i++) {
		hv_cdev_device_num = MKDEV(hv_cdev_major[cdev_type], HV_CDEV_FIRST_MINOR+i);
		device_destroy(devices[cdev_type][i].hv_cdev_class, hv_cdev_device_num);
		class_unregister(devices[cdev_type][i].hv_cdev_class);
		class_destroy(devices[cdev_type][i].hv_cdev_class);
		cdev_del(&devices[cdev_type][i].cdev);
	}
