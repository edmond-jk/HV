...
SW_VERSION=0321
OPTION='NO'
REG07=0x53			# V82 3/28/17 from 0x53
...
#V82 3/28/17
	./test$SW_VERSION -d $DIMM -t 0x800 0 128 4 -u
	if [ "$?" = "1" ]; then
...
#	./test$SW_VERSION -d $DIMM -t 16 20 132 2 -D 1 -u	# 2/14/2017
	./test$SW_VERSION -d $DIMM -t 16 0 128 4 -D 1 -u	# 3/13/2017, data size 1MB from 4KB
...
#	./test$SW_VERSION -d $DIMM -R1
	...
	echo "*** DIMM[$DIMM] - TxDQDQS2_random(FakeWR) training & update H/W *******************"
	./test$SW_VERSION -d $DIMM -t 0x1000 0 128 4 -u		# 3/21/2017, random data

	echo "*** DIMM[$DIMM] - Check General Write Status **************************************"
	./test$SW_VERSION -d $DIMM -FW 10
