/*
 * HVDIMM training example code by SJ Choi 1/26/2016
 *
 * test0715 -d 3 -u -l -s 0 -e 180 -z 4 -t 0x4 -f tx.log
 *
 * 
 */
#define VERSION "-7.30.16"
...
//MK-begin
#define MMIO_WINDOW_SIZE			0x100000	// 1MB
#define MMIO_WINDOW_SIZE_2			0x40000000	// 1GB
//MK-end
...
//
#define FPGA_WRITE_DLY		100	// usleep(), 1000 = 1ms 
#define FPGA_CMD_WR_DLY		10000	// usleep(), 10000 = 10ms 
#define FPGA_SETPAGE_DLY	10000	// usleep(), 10000 = 10ms 
...
unsigned long demo_qwdata1, demo_qwdata2;
///MK0730-end
...
int colorlimit=20, testtime=0, setpattern=0;
...
char *debug_str=NULL;
//MK0803-begin
char *demo_debug_str=NULL;
unsigned int demo_debug_flag=0;
//MK0803-end
...
//MK-end
//MK0803-begin
unsigned long predefined_pattern[8] = {
	0x8765432187654321,
	0x1111111188888888,
	0x55555555AAAAAAAA,
	0x5555444433332222,
...
//MK0803-begin
int presight_demo(int dimm_num, unsigned long qwcnt, unsigned long repeat_cnt);
int TxDQDQS2_demo(int dimm_num, unsigned long *pdst, unsigned long repeat_count);
void showPresightmemory(int dimm_num);
//MK0803-end
...
//MK0730-begin
void MemCpy3(int rwflag, unsigned long *pbuff, unsigned long data1, unsigned int qwcnt)
{
	int i;
	unsigned long qwdata, *ptemp=pbuff;
	...
	for (i=0; i < qwcnt; i++)
	{
		...
		if(rwflag == WR) {
			/* 64-bit write */
			if (demo_debug_flag & 0x00000001)
				printf("[MemCpy3]: WR: pbuff=0x%.16lx data=0x%.16lx\n", (unsigned long)ptemp, data1);

			*ptemp++ = data1;
		} else {
			/*
			 * 64-bit read, we just want to generate read transactions.
			 * Not interested in actual values
			 */
			qwdata = *ptemp++;
			if (demo_debug_flag & 0x00000001)
				printf("[MemCpy3]: RD: pbuff=0x%.16lx data=0x%.16lx\n", (unsigned long)(ptemp-1), qwdata);
		}
	}

	clflush_cache_range(pbuff, qwcnt*8);
}
//MK0730-end
	...
	for (i=start_lp; i<end_lp; i=i+step_size) {
		...
		if ( ((ts_5 & TRN_MODE)==TRN_MODE) && ((ts_5 & TRN_ERR)==0) 
		  ...
		  && ((ts_7 & TRN_MODE)==TRN_MODE) && ((ts_7 & TRN_ERR)==0) 
			...
			printf("CMDCK:: FPGA1/2 is not in test mode(A[103] bit0 is 0)!\n");
			exit (-1);
		}
	...
	for (i=start_lp; i < end_lp; i=i+step_size) {
		...
		if ( ((ts_5 & TRN_MODE)==TRN_MODE) && ((ts_5 & TRN_ERR)==0) 
		  ...
		  && ((ts_7 & TRN_MODE)==TRN_MODE) && ((ts_7 & TRN_ERR)==0) 
			...
			printf("\n\tTxDQDQS1:: FPAG1/2 is not in test mode (A[103] bit0 is 0)!\n");
			exit (-1);
		}
	...
	for(i=start_lp; i<end_lp; i=i+step_size) {
		...
		if ( ((ts_5 & TRN_MODE)==TRN_MODE) && ((ts_5 & TRN_ERR)==0) 
		  ...
		  && ((ts_7 & TRN_MODE)==TRN_MODE) && ((ts_7 & TRN_ERR)==0) 
			...
			printf("RxDQDQS1:: FPAG1/2 is not in test mode (A[103] bit0 is 0)!\n");
			exit (-1);
		}
...
//		if (byteData & 0x01 == 1) {
		...
		if ( ((ts_5 & TRN_MODE)==TRN_MODE) && ((ts_5 & TRN_ERR)==0)
		  ...
		  && ((ts_7 & TRN_MODE)==TRN_MODE) && ((ts_7 & TRN_ERR)==0)
			...
			printf("TxDQDQS2:: FPGA1/2 is not in test mode (A[103] bit0 is 0)!\n");
			exit (-1);
		}
	...
	//	  && ((ts_7 & 0x84) ^ 0x80 == 0x84)
		  && ((ts_7 & CAPTURE_DN) == CAPTURE_DN) && ((ts_7 & TRN_ERR) == 0)
			...
			qw1xor = qw1 ^ (data3 << 32 | data1) ;		// RD data1 ^ expected Data1
			qw2xor = qw2 ^ (data4 << 32 | data2) ;		// RD data2 ^ expected Data2
...
//MK0801-begin
/**
 * TxDQDQS2_demo - FPGA-to-DDR4 Write Training for fake-write operation
 *
 * This routine is only for 8/9 demo
 *
 **/
int TxDQDQS2_demo(int dimm_num, unsigned long *pdst, unsigned long repeat_count)
{
	int	ret_code=0;
	unsigned int j;
	unsigned long qwbuff[8];
//MK0804-begin
	unsigned long *ptemp;
	unsigned char byte_data1[8], byte_data2[8];
//MK0804-end

	/* 1. Set training mode to 3 for Tx DQ-DQS(2) */
	WriteSMBus(FPGA1, dimm_num, TRN_MODE_ADDR, TX_DQDQS_2_MODE);
#if BOTH_FPGA
	WriteSMBus(FPGA2, dimm_num, TRN_MODE_ADDR, TX_DQDQS_2_MODE);
#endif
	usleep(FPGA_CMD_WR_DLY);

	/* 2. Write a pattern to training_data window for Fake WR operation */
//	MemCpy2(WR, 0x6666666666666666, 0x9999999999999999, TX_DQDQS2_DATA_SIZE);
//MK0804	MemCpy3(WR, (unsigned long *)pdst, demo_qwdata, 8);
//MK0804-begin
//	qwbuff[0] = demo_qwdata1;		// First 64-bit = 8 bytes
//	qwbuff[1] = demo_qwdata2;		// Second 64-bit = 8 bytes
//	printf("pdst=0x%.16lx qwbuff[0]=0x%.16lx qwbuff[1]=0x%.16lx\n", (unsigned long)pdst, qwbuff[0], qwbuff[1]);
//	memcpy((void *)pdst, (void *)qwbuff, 64);	// Total 16 bytes
	for (j=0; j < 7; j++)
	{
		*(pdst+j) = 0xF1F1F1F1F1F1F1F1;		// write 8 dummy qwords
	}
//MK0804-end

	/* 3. Set training mode to NORMAL mode */
//	WriteSMBus(FPGA1, 3, TRN_MODE_ADDR, NORMAL_MODE);
#if BOTH_FPGA
//	WriteSMBus(FPGA2, 3, TRN_MODE_ADDR, NORMAL_MODE);
#endif
//	usleep(FPGA_CMD_WR_DLY);


	/* Show the result of the presight-write operation */
	for (j=0; j < 7; j++)
	{
		qwbuff[j] = *(pdst+j);
	}
//	for (j=0; j < 7; j++)
//MK0808	for (j=0; j < 2; j++)
//MK0808	{
//MK0808		printf("[0x%.16lx] 0x%.16lx\n", (unsigned long)(pdst+j), qwbuff[j]);
//MK0808	}

//MK0804-begin
	for (j=0; j < 4; j++)
	{
		byte_data2[j] = ReadSMBus(FPGA1, dimm_num, 115+j);
		byte_data2[j+4] = ReadSMBus(FPGA2, dimm_num, 115+j);
		byte_data1[j] = ReadSMBus(FPGA1, dimm_num, 119+j);
		byte_data1[j+4] = ReadSMBus(FPGA2, dimm_num, 119+j);
	}

//	printf("byte_data1=0x%.16lx\n", *(unsigned long *)byte_data1);
//	printf("byte_data2=0x%.16lx\n", *(unsigned long *)byte_data2);
//	for (j=0; j < 7; j++)
	for (j=0; j < 2; j++)
	{
		if (j % 2 == 0) {
			if (*(unsigned long *)(pdst+j) != *(unsigned long *)byte_data1) {
//MK0808				printf("qword index=%d expected pattern=0x%.16lx pattern from DRAM=0x%.16lx dram addr=0x%.16lx\n",
//MK0808						j, *(unsigned long *)byte_data1, *(unsigned long *)(pdst+j), (unsigned long)(pdst+j));
//MK0808				ret_code = -1;
//MK0808				break;
//MK0808-begin
				printf("[0x%.16lx] 0x%.16lx\n", (unsigned long)(pdst+j), *(unsigned long *)byte_data1);
//MK0808-end

			}
//MK0808-begin
			else {
				printf("[0x%.16lx] 0x%.16lx\n", (unsigned long)(pdst+j), *(unsigned long *)(pdst+j));
			}
//MK0808-end
		} else {
			if (*(unsigned long *)(pdst+j) != *(unsigned long *)byte_data2) {
//MK0808				printf("qword index=%d expected pattern=0x%.16lx pattern from DRAM=0x%.16lx dram addr=0x%.16lx\n",
//MK0808						j, *(unsigned long *)byte_data2, *(unsigned long *)(pdst+j), (unsigned long)(pdst+j));
//MK0808				ret_code = -1;
//MK0808				break;
//MK0808-begin
				printf("[0x%.16lx] 0x%.16lx\n", (unsigned long)(pdst+j), *(unsigned long *)byte_data2);
//MK0808-end
			}
//MK0808-begin
			else {
...
void showPresightMemory(int dimm_num)
{
	...
	for (i=0; i < 4; i++)
	{
		byte_data2[i] = ReadSMBus(FPGA1, dimm_num, 115+i);
		byte_data2[i+4] = ReadSMBus(FPGA2, dimm_num, 115+i);
		byte_data1[i] = ReadSMBus(FPGA1, dimm_num, 119+i);
		byte_data1[i+4] = ReadSMBus(FPGA2, dimm_num, 119+i);
	}
	printf("0x%.16lx\n", *(unsigned long*)byte_data1);
	printf("0x%.16lx\n", *(unsigned long*)byte_data2);
}
...
//MK0730-begin
/**
 * presight_demo - Demonstrate fake-write operation with block level image
 *
 **/
int presight_demo(int dimm_num, unsigned long qwcnt, unsigned long repeat_count)
{
	unsigned long *v_addr;
//	unsigned char fpga1reg[32], fpga2reg[32];
	unsigned int i, j, master32bit[2], slave32bit[2];
	unsigned long rand_pattern;
	unsigned char byte_data1[8], byte_data2[8];
//MK0804-begin
	int ret_code=0;
//MK0804-end
	...
	if (demo_action == 1) {
		...
		/* Get the base addr of MMIO CMD WRT training_data window */
		v_addr = (unsigned long *) (map_base + (target & MAP_MASK2) + CMD_OFFSET);

		if (demo_debug_flag & 0x00000001)
			printf("[presight_demo]: target=0x%.16lx map_base=0x%.16lx v_addr=0x%.16lx\n", target+CMD_OFFSET, map_base, v_addr);

		WriteSMBus(FPGA1, dimm_num, 6, 0x00);
		usleep(FPGA_WRITE_DLY);
		WriteSMBus(FPGA2, dimm_num, 6, 0x00);
		usleep(FPGA_WRITE_DLY);

		/* 1. Write a demo pattern to MMIO window (64 bytes) */
//MK0804		MemCpy3(WR, (unsigned long *)v_addr, demo_qwdata1, 8);

		/*
		 * Split the input 64-bit data into two 32-bit variables. The lower 32-bit
		 * belongs to the master FPGA and the upper belongs to the slave FPGA.
		 * Write this pattern to presight-write data regs. This step is needed
		 * because the command data pattern is not captured by FPGA all the time.
		 */
		master32bit[0] = (unsigned int) demo_qwdata1;			// lower 32-bit
		slave32bit[0] = (unsigned int) (demo_qwdata1 >> 32);	// upper 32-bit
//MK0804		master32bit[1] = master32bit[0];
//MK0804		slave32bit[1] = slave32bit[0];
//MK0804-begin
		master32bit[1] = (unsigned int) demo_qwdata2;			// lower 32-bit
		slave32bit[1] = (unsigned int) (demo_qwdata2 >> 32);	// upper 32-bit
//MK0804-end
		for (i=0; i < 4; i++)
		{
			WriteSMBus(FPGA1, dimm_num, 115+i, (master32bit[0] >> i*8) & 0x000000FF);
			WriteSMBus(FPGA2, dimm_num, 115+i, (slave32bit[0] >> i*8) & 0x000000FF);
			usleep(FPGA_WRITE_DLY);
			WriteSMBus(FPGA1, dimm_num, 119+i, (master32bit[1] >> i*8) & 0x000000FF);
			WriteSMBus(FPGA2, dimm_num, 119+i, (slave32bit[1] >> i*8) & 0x000000FF);
			usleep(FPGA_WRITE_DLY);
		}

	} else if (demo_action == 2) {

		/* Get the base addr of presight-write training_data window */
//MK0804		v_addr = map_base + (target & MAP_MASK2);
//MK0804-begin
		v_addr = (unsigned long *) (map_base + (target & MAP_MASK2));
//MK0804-end

		if (demo_debug_flag & 0x00000001)
			printf("[presight_demo]: target=0x%.16lx map_base=0x%.16lx v_addr=0x%.16lx\n", target, map_base, v_addr);

#if 0	//MK0802
		/* 2. Set demo mode to FPGA register 6 */
		WriteSMBus(FPGA1, dimm_num, 6, 0x01);
		usleep(FPGA_WRITE_DLY);
		WriteSMBus(FPGA2, dimm_num, 6, 0x01);
		usleep(FPGA_WRITE_DLY);

		printf("FPGA1-3-6=%d FPGA2-3-6=%d\n", ReadSMBus(FPGA1, dimm_num, 6), ReadSMBus(FPGA2, dimm_num, 6));

		/* 3. Write 64-byte pattern to an arbitrary address for Fake WR */
		MemCpy3(WR, (unsigned long *)v_addr, demo_qwdata, 8);


		/* 2. Set demo mode to FPGA register 6 */
		WriteSMBus(FPGA1, dimm_num, 6, 0);
		usleep(FPGA_WRITE_DLY);
		WriteSMBus(FPGA2, dimm_num, 6, 0);
		usleep(FPGA_WRITE_DLY);
#endif	//MK0802

//MK0802-begin
		ret_code = TxDQDQS2_demo(dimm_num, v_addr, 1);
//MK0802-end

	} else if (demo_action == 3) {

		showPresightMemory(dimm_num);
		...
		{
			v_addr = map_base + (target & MAP_MASK2);
			printf("v_addr=0x%.16lx test pattern=0x%.16lx\n",
					(unsigned long)v_addr, predefined_pattern[pattern_index % 8]);

			// generate a random 64-bit pattern
			// write the pattern to FPGA regs like "demo action 1"
			master32bit[0] = (unsigned int) predefined_pattern[pattern_index % 8];			// lower 32-bit
			slave32bit[0] = (unsigned int) (predefined_pattern[pattern_index % 8] >> 32);	// upper 32-bit
			master32bit[1] = master32bit[0];
			slave32bit[1] = slave32bit[0];
			for (i=0; i < 4; i++)
			{
				WriteSMBus(FPGA1, dimm_num, 115+i, (master32bit[0] >> i*8) & 0x000000FF);
				WriteSMBus(FPGA2, dimm_num, 115+i, (slave32bit[0] >> i*8) & 0x000000FF);
			...
			pattern_index++;
			TxDQDQS2_demo(dimm_num, v_addr, 1);

			for (i=0; i < 4; i++)
			{
				byte_data1[i] = ReadSMBus(FPGA1, dimm_num, 115+i);
				byte_data1[i+4] = ReadSMBus(FPGA2, dimm_num, 115+i);
				byte_data2[i] = ReadSMBus(FPGA1, dimm_num, 119+i);
				byte_data2[i+4] = ReadSMBus(FPGA2, dimm_num, 119+i);
			}

			for (i=0; i < 7; i++)
			{
				if (*(unsigned long *)(v_addr+i) != *(unsigned long *)byte_data1) {
					printf("qword index=%d expected pattern=0x%.16lx pattern from DRAM=0x%.16lx dram addr=0x%.16lx\n",
							i, *(unsigned long *)byte_data1, *(unsigned long *)(v_addr+i), (unsigned long)(v_addr+i));
//MK0804					return -1;
//MK0804-begin
					ret_code = -1;
					goto exit;
//MK0804-end
		...
		printf("%d iterations of presight write/read completed successfully!!\n", j);

	} else {
		/* 4. Set training mode to NORMAL mode */
		WriteSMBus(FPGA1, dimm_num, 6, 0);
		usleep(FPGA_WRITE_DLY);
		WriteSMBus(FPGA2, dimm_num, 6, 0);
		usleep(FPGA_WRITE_DLY);
...
#if 0		
		"\n  -om --offset_minus <tid> <value>"
		"\n  -op --offset_plus <tid> <value>"
#endif
...
//MK0803-begin
	/* Get debug instruction from the user */
	if ( (demo_debug_str = getenv("HVT_DEMO_DEBUG")) != NULL )
		demo_debug_flag = atoi(demo_debug_str);
//MK0803-end
...
//			step_size16 = atoi(argv[i+1]);
		...
		if (strcmp (arg, "-op") == 0 || strcmp (arg, "--offset_plus") == 0) {
			toffadj = 2;
			Training = strtoul(argv[i+1],0,0);
			data = strtoul(argv[i+2],0,0);
		}		
...
//MK0730-begin
	if (demo_flag) {
		if (demo_debug_flag & 0x00000001)
			printf("[-demo debug]: tohm=0x%.16lx target=0x%.16lx map_base=0x%.16lx\n", tohm, target, map_base);

		ret_code = presight_demo(D, 8, demo_repeat_count);	// dimm#=3, qwcnt=8, repeat_cnt=1
		goto exit;
	}
//MK0730-end
	...
	if(toffadj) {
		printf("\n DIMM[%d] Training(%d) Data offset adjust %c%d tick(s) ... ", 
						D, Training, (toffadj==1)?'-':'+', data);	
		if( Training ==4 ) {
			for(i=0;i<64+eccMode*8;i++) {
				tdata = ReadSMBus( (i2cTXDQS1_perBit[i]>>12), D, i2cTXDQS1_perBit[i]&0xFF);
				if(toffadj == 1) tdata -= data;
				else			 tdata += data;
				if(tdata<0) { printf(" addr= 0x%x, adj value(%d) is negative ... ERRO !!!\n\n",
		...
		if( Training == 0x10 ) {
			...
			usleep(10000);
			for(i=0;i<64+eccMode*8;i++) {
				tdata = ReadSMBus( (i2cTXDQS2_perBit[i]>>12), D, i2cTXDQS2_perBit[i]&0xFF);
				if(toffadj == 1) tdata -= data;
				else			 tdata += data;
				if(tdata<0) { printf(" addr= 0x%x, adj value(%d) is negative ... ERRO !!!\n\n",
								i2cTXDQS2_perBit[i]&0xFF, tdata ); exit(1);
				}				
	...
	if(bcomsw) { enable_bcom_switch(); exit(1); }
